import React from 'react';
import { View, Text, Pressable, ScrollView, Modal, StyleSheet } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import * as Haptics from 'expo-haptics';
import { LinearGradient } from 'expo-linear-gradient';
import { useColorScheme } from '@/components/useColorScheme';
import { useSettingsStore } from '@/src/store/settings-store';
import { useAuthStore } from '@/src/store/auth-store';
import { useUserProfileStore } from '@/src/store/user-profile-store';
import { battleCharacters, type BattleCharacter } from '@/src/data/content/battle-characters';
import { useRouter } from 'expo-router';

interface BattleCharacterModalProps {
  visible: boolean;
  onClose: () => void;
  onSelectCharacter: (character: BattleCharacter) => void;
}

const DIFFICULTY_COLORS = {
  easy: '#22C55E',
  medium: '#F59E0B',
  hard: '#EF4444',
};

const DIFFICULTY_LABELS = {
  easy: { en: 'Easy', de: 'Leicht' },
  medium: { en: 'Medium', de: 'Mittel' },
  hard: { en: 'Hard', de: 'Schwer' },
};

export function BattleCharacterModal({ visible, onClose, onSelectCharacter }: BattleCharacterModalProps) {
  const colorScheme = useColorScheme();
  const isDark = colorScheme === 'dark';
  const locale = useSettingsStore((s) => s.locale);
  const isPremium = useAuthStore((s) => s.isPremium);
  const userGender = useUserProfileStore((s) => s.userGender);
  const router = useRouter();

  // Show opponents matching the opposite gender (or all for diverse)
  const filteredCharacters = React.useMemo(() => {
    if (!userGender || userGender === 'diverse') return battleCharacters;
    // Male users see female opponents, female users see male opponents
    return battleCharacters.filter((c) => c.forGender !== userGender);
  }, [userGender]);

  const handleSelect = (character: BattleCharacter) => {
    if (character.isPremium && !isPremium) {
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
      router.push('/paywall');
      onClose();
      return;
    }
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
    onSelectCharacter(character);
    onClose();
  };

  return (
    <Modal
      visible={visible}
      animationType="slide"
      presentationStyle="pageSheet"
      onRequestClose={onClose}
    >
      <SafeAreaView style={[styles.safeArea, isDark && styles.safeAreaDark]}>
        <View style={styles.header}>
          <Text style={[styles.title, isDark && styles.titleDark]}>
            {locale === 'de' ? 'Flirt Battle' : 'Flirting Battle'}
          </Text>
          <Pressable onPress={onClose} style={[styles.closeBtn, isDark && styles.closeBtnDark]}>
            <Ionicons name="close" size={24} color={isDark ? '#A3A3A3' : '#737373'} />
          </Pressable>
        </View>
        <Text style={[styles.subtitle, isDark && styles.subtitleDark]}>
          {locale === 'de'
            ? 'Wähle deinen Gegner. 10 Nachrichten, dann bekommst du deinen Score.'
            : 'Choose your opponent. 10 messages, then you get your score.'}
        </Text>

        <ScrollView contentContainerStyle={styles.characterList} showsVerticalScrollIndicator={false}>
          {filteredCharacters.map((char) => {
            const isLocked = char.isPremium && !isPremium;
            const diffColor = DIFFICULTY_COLORS[char.difficulty];

            return (
              <Pressable
                key={char.id}
                onPress={() => handleSelect(char)}
                style={[styles.card, isDark && styles.cardDark]}
              >
                <View style={styles.cardHeader}>
                  <View style={[styles.iconBox, { backgroundColor: `${char.color}15` }]}>
                    <Ionicons name={char.icon as any} size={24} color={char.color} />
                  </View>
                  <View style={styles.cardInfo}>
                    <View style={styles.nameRow}>
                      <Text style={[styles.name, isDark && styles.nameDark]}>{char.name}</Text>
                      <View style={[styles.diffBadge, { backgroundColor: `${diffColor}15` }]}>
                        <Text style={[styles.diffText, { color: diffColor }]}>
                          {DIFFICULTY_LABELS[char.difficulty][locale]}
                        </Text>
                      </View>
                      {isLocked && (
                        <View style={styles.proBadge}>
                          <Ionicons name="lock-closed" size={10} color="#E8435A" />
                          <Text style={styles.proText}>PRO</Text>
                        </View>
                      )}
                    </View>
                    <Text style={[styles.charSubtitle, isDark && styles.charSubtitleDark]}>
                      {char.subtitle[locale]}
                    </Text>
                  </View>
                </View>
                <Text style={[styles.description, isDark && styles.descriptionDark]}>
                  {char.description[locale]}
                </Text>
              </Pressable>
            );
          })}
          <View style={{ height: 40 }} />
        </ScrollView>
      </SafeAreaView>
    </Modal>
  );
}

const styles = StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: '#FAFAFA' },
  safeAreaDark: { backgroundColor: '#171717' },
  header: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
    paddingHorizontal: 20, paddingTop: 20, paddingBottom: 4,
  },
  title: { fontSize: 28, fontWeight: '700', color: '#171717', letterSpacing: -0.5 },
  titleDark: { color: '#F5F5F5' },
  closeBtn: {
    width: 36, height: 36, borderRadius: 18,
    backgroundColor: 'rgba(0,0,0,0.05)',
    alignItems: 'center', justifyContent: 'center',
  },
  closeBtnDark: { backgroundColor: 'rgba(255,255,255,0.08)' },
  subtitle: {
    fontSize: 15, color: '#737373', paddingHorizontal: 20, marginTop: 4, marginBottom: 20,
  },
  subtitleDark: { color: '#A3A3A3' },
  characterList: { paddingHorizontal: 20, gap: 12 },
  card: {
    backgroundColor: '#fff', borderRadius: 20, padding: 18,
    borderWidth: StyleSheet.hairlineWidth, borderColor: 'rgba(0,0,0,0.06)',
    shadowColor: '#000', shadowOffset: { width: 0, height: 2 },
    shadowOpacity: 0.04, shadowRadius: 8, elevation: 2,
  },
  cardDark: {
    backgroundColor: '#252525', borderColor: 'rgba(255,255,255,0.08)',
  },
  cardHeader: { flexDirection: 'row', alignItems: 'center', gap: 14, marginBottom: 10 },
  iconBox: {
    width: 48, height: 48, borderRadius: 16,
    alignItems: 'center', justifyContent: 'center',
  },
  cardInfo: { flex: 1 },
  nameRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  name: { fontSize: 18, fontWeight: '700', color: '#171717', letterSpacing: -0.2 },
  nameDark: { color: '#F5F5F5' },
  diffBadge: {
    paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8,
  },
  diffText: { fontSize: 10, fontWeight: '700' },
  proBadge: {
    flexDirection: 'row', alignItems: 'center', gap: 3,
    backgroundColor: 'rgba(232,67,90,0.08)',
    paddingHorizontal: 8, paddingVertical: 3, borderRadius: 8,
  },
  proText: { fontSize: 10, fontWeight: '800', color: '#E8435A' },
  charSubtitle: { fontSize: 13, color: '#737373', marginTop: 2 },
  charSubtitleDark: { color: '#A3A3A3' },
  description: { fontSize: 14, lineHeight: 20, color: '#525252' },
  descriptionDark: { color: '#A3A3A3' },
});
